...
from models.resnet_gaze.model_gaze import GazeLSTM
...
class Trainer():
    ...
    def __init__(self, model_name, dataset_name, classes_num, input_img_size, channel_num,
        ...
        model_name_list = ['Face_cls', 'face_feature', 'maskrcnn', 'inception', 'inception4', 'Head_posture',
                           'DeepFace', 'Gaze360', 'resnet18','sp_model','openpose','sp_model_4','sp_model_5',
                           'resnet34', 'art_pic',
                           'resnet50',
                           'resnet101', 'resnet152', 'resnext50_32x4d', 'resnext101_32x8d', 'wide_resnet50_2',
                           'wide_resnet101_2'] + \
                          ['vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg16', 'vgg16_bn', 'vgg19', 'vgg19_bn'] + \
                          ['alexnet'] + ['densenet121', 'densenet161', 'densenet169', 'densenet201'] + ['googlenet'] + [
                              'mobilenet_v2'] + ['mnasnet0_5', 'mnasnet1_0']
    ...
    def logging_dataset_info(self):
        ...
        '''
        save_txt(self.txt_path, "",
                 "Dataset Name : {} , number of classes : {} , input image size : {} , channel number : {}"
                 .format(self.dataset_name, self.classes_num, self.input_img_size, self.channel_num), "")

        save_txt(self.txt_path,
                 "No. of images in train and val dataset : {} , {}".format(self.train_img_num, self.val_img_num), "")
        ...
        show_image_batch(self.train_loader,
                         os.path.join(self.result_main_path, "Augmentation_Result_Batch.png").replace('\\', '/'),
                         self.mean, self.std, self.class_names)  # show images for one batch after transform

        show_image(self.train_folders,
                   os.path.join(self.result_main_path, "Augmentation_Result.png").replace('\\', '/'),
    ...
    def train(self, model, criterion, optimizer, scheduler, early_stopping_patience):
        ...
        save_txt(self.txt_path,
                 "No. of model parameters [trainable parameters, total] : " + str(self.count_parameters(model)), "")
        save_txt(self.txt_path, "=" * 50, "")
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for batch_index, (img, label) in enumerate(self.train_loader):
                ...
                _, predicted = torch.max(output.data,
                                         1)  # output.data is similar to output but "requires_grad" = False. In other words, only the value or data of Tensor. Data-type of "output.data" is still Tensor.
                ...
                #                                                                        (
                #                                                                                predicted == label.data).sum().item() / label.size(
                #                                                                            0), loss.item()))
        ...
        # print('Is model on Cuda? : ', next(model.parameters()).is_cuda)
        torch.save(model.state_dict(),
                   os.path.join(self.result_main_path, '{}-bestmodel.pth'.format(self.model_name)).replace('\\', '/'))
        ...
        plot_graph(train_acc_dict, "train accuracy", "epoch", "acc",
                   os.path.join(self.result_main_path, "train_acc.png"))
        plot_graph(train_loss_dict, "train loss", "epoch", "loss",
                   os.path.join(self.result_main_path, "train_loss.png"))
    ...
    def build_model(self, ImageNet_pretrained):
        ...
        if self.model_name in ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152', 'resnext50_32x4d',
            ...
            if not ImageNet_pretrained:
                ...
                model.fc = nn.Linear(num_ftrs,
                                     self.classes_num)  # modify fc layer , No. of outputs is set to self.classes_num
                return model
        ...
        if self.model_name in ['vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg16', 'vgg16_bn', 'vgg19', 'vgg19_bn']:
            ...
            if not ImageNet_pretrained:
                ...
                num_ftrs = model.classifier[
                    6].in_features  # No. of input features to fully connected layer, last linear layer
                model.classifier[6] = nn.Linear(num_ftrs,
                                                self.classes_num)  # modify fc layer , No. of outputs is set to self.classes_num
                return model
        ...
        if self.model_name in ['densenet121', 'densenet161', 'densenet169', 'densenet201']:
            ...
            if not ImageNet_pretrained:
                ...
                model.classifier = nn.Linear(num_ftrs,
                                             self.classes_num)  # modify fc layer , No. of outputs is set to self.classes_num
                return model
        ...
        if self.model_name in ['mobilenet_v2']:
            ...
            if not ImageNet_pretrained:
                ...
                model = model_object(pretrained=ImageNet_pretrained, num_classes=1000)
                num_ftrs = model.classifier[
                    1].in_features  # No. of input features to fully connected layer, last linear layer
                model.classifier[1] = nn.Linear(num_ftrs,
                                                self.classes_num)  # modify fc layer , No. of outputs is set to self.classes_num
                return model
    ...
    def load_model(self, model_path):
        ...
        '''
        if self.model_name in ['Head_posture']:
            DEPTH = 18
            MAX_SIZE = 1400
            MIN_SIZE = 600
            threed_points = np.load('models/Head_pre/pose_references/reference_3d_68_points_trans.npy')
            POSE_MEAN = "models/Head_pre/models/WIDER_train_pose_mean_v1.npy"
            POSE_STDDEV = "models/Head_pre/models/WIDER_train_pose_stddev_v1.npy"
            MODEL_PATH = "models/Head_pre/models/img2pose_v1.pth"
            ...
            load_model(img2pose_model.fpn_model, MODEL_PATH, cpu_mode=str(img2pose_model.device) == "cpu",
                       model_only=True)
            img2pose_model.evaluate()
            model_ex = tx.Extractor(img2pose_model.fpn_model, ['module.roi_heads.box_head.fc7'])
            return model_ex

        if self.model_name in ['Gaze360']:
            model = GazeLSTM()
    ...
    def extract_features_single_image(self, image_path, model, model_feature):
        ...
        img_trans = self.val_transforms(img).reshape(1, self.channel_num, self.input_img_size[0],
            ...
            for sub_folder in directories:
                for _, _, filenames in os.walk(os.path.join(DatasetPath, sub_folder)):
                    for filename in filenames:
                        yield filename, sub_folder, os.path.join(DatasetPath, sub_folder, filename).replace('\\',
                                                                                                            '/')  # image filename, subfolder name, image path
            break
    ...
    def img_loader_f(self, DatasetPath):  # input images should be stored in one folder (Dataset)
        for root, _, filenames in os.walk(DatasetPath):
            for filename in filenames:
                yield filename, os.path.join(DatasetPath, filename).replace('\\',
...
class Predictor():
    ...
    def __init__(self, model_name, classes_num, input_img_size, channel_num, result_folder,
        ...
        model_name_list = ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152', 'resnext50_32x4d',
                           'resnext101_32x8d', 'wide_resnet50_2', 'wide_resnet101_2'] + \
                          ['vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg16', 'vgg16_bn', 'vgg19', 'vgg19_bn'] + \
                          ['alexnet'] + ['densenet121', 'densenet161', 'densenet169', 'densenet201'] + ['googlenet'] + [
                              'mobilenet_v2'] + ['mnasnet0_5', 'mnasnet1_0']
    ...
    def predict_single_image(self, image_path, model):
        ...
        img = Image.open(image_path)
        img_trans = self.test_transforms(img).reshape(1, self.channel_num, self.input_img_size[0],
                                                      self.input_img_size[0])
        '''
        If you don't want to use channel_num here, use following code:
    ...
    def predict_dataset(self, dataset_path, model, criterion, plot_cm=False):
        ...
        with torch.no_grad():  # temporarily set all the requires_grad flags to false (no gradient calculations).
            ...
            for batch_index, (img, label) in enumerate(dataset_loader):
                ...
                predicted_all = np.append(predicted_all,
                                          predicted.cpu().data.numpy())  # put Tensor on CPU, then convert to numpy array
        ...
        visualize_prediction(dataset_path,
                             os.path.join(self.result_main_path, "Prediction_Result.png").replace('\\', '/'),
                             model, self.model_name, self.test_transforms, self.mean, self.std, self.cuda,
                             images_num=12, row=2, title='Model Prediction')

        if plot_cm:
            plot_confusion_matrix(cm, class_names,
                                  os.path.join(self.result_main_path, "Prediction_cm.png").replace('\\', '/'),
                                  normalize=True)
