...
def gaze_transform_deal(fimg,mean=mean, std=std):
    image_normalize = transforms.Normalize(mean=mean, std=std)
    input_image = torch.zeros(7, 3, 224, 224)
    for i in range(7):
        input_image[i, :, :, :] = image_normalize(transforms.ToTensor()(transforms.Resize((224, 224))(fimg)))
    input_image = input_image.view(1, 7, 3, 224, 224)
    return input_image

def get_test_transform(mean=mean, std=std):
